...
class Switch {
	...
	constructor( network, discoverPacket ) {
		...
		this._netmask = discoverPacket.body.netmask[0];
		this._gateway = discoverPacket.body.gateway[0];
		this._vlan_support = discoverPacket.body.vlan_support[0];
	...
	get info() { return {
		...
		vlan_support: this._vlan_support
	}; }
	...
	setName( name ) { return this._network._getSockets().then( ( s ) => {
